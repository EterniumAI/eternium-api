...
async function fetchCreativeWithAccount(env, creativeId) {
	const qs = new URLSearchParams({
		select: 'id,ads_account_id,status,external_id,created_at,name',
		id: `eq.${creativeId}`,
	});
...
export async function handleRetireCreative(creativeId, body, env, keyData) {
	...
	// Pause on Meta
	const metaResult = await pauseMetaAd(env, creative.external_id);
	if (!metaResult.ok) {
		...
		await writeFleetEvent(env, 'creative_retire_failed', {
			...
			meta_ad_id: creative.external_id,
...
export async function handlePromoteCreative(creativeId, body, env, keyData) {
	...
	if (champions.length >= 2) {
		...
		if (championToDemote) {
			...
			// If retire fails (e.g. no external_id), still proceed with promote

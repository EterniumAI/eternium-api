...
// ── LLM caller: delegates to shared runInternalChatCompletion helper ────────

import { runInternalChatCompletion } from './internal-llm.js';
...
async function callLLM(env, prompt) {
	...
	const result = await runInternalChatCompletion(env, {
	...
	if (!result.ok) {
		return { result: null, error: result.error, tokenUsage: null };
	}

	const text = result.data.choices?.[0]?.message?.content || '';
	return { result: text, error: null, tokenUsage: result.tokenUsage };
}

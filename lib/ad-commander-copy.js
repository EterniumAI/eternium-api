 ...
 *
 * Uses Claude API via fetch (no SDK dependency).
 */
...
Return strictly a JSON array of 3 objects with fields: headline (string),
body_copy (string), cta_type (enum), rationale (one sentence explaining
why this variant will perform). No preamble, no trailing commentary,
just the JSON array.`;
...
export async function handleAdCommanderDraft(body, env, keyData) {
	...
	// Call Claude
	const { result, error: claudeError, tokenUsage } = await callClaude(env, prompt);

	if (claudeError) {
		return { code: 503, data: { error: claudeError, code: 'CLAUDE_UPSTREAM' } };
	}
	...
	try {
		...
		return { code: 502, data: { error: 'Failed to parse Claude response', code: 'PARSE_ERROR' } };
	}

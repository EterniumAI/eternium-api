...
export async function assertAdCommanderAccess(env, adsAccountId, keyData) {
	const qs = new URLSearchParams({
		select: 'tenant_id,project_id,projects(id,project_type)',
		id: `eq.${adsAccountId}`,
	});
	...
	if (!allowedTypes.includes(project.project_type)) {
	...
	if (!isAdmin && row.tenant_id !== keyData?.tenant_id) {
		return { ok: false, code: 403, error: 'Access denied to this ads account', detail: null };
	}
...
async function fetchTopWinners(env, adsAccountId) {
	...
	const qs = new URLSearchParams({
		select: 'creative_id,clicks,impressions,date,ads_creatives(id,headline,body_copy,cta_type)',
...
// ── LLM caller: OpenAI primary, OpenRouter fallback (matches /v1/chat upstream)
...
async function callLLM(env, prompt) {
	if (!env.OPENAI_API_KEY && !env.OPENROUTER_API_KEY) {
		return { result: null, error: 'No LLM upstream configured', tokenUsage: null };
	}
	...
	if (env.OPENAI_API_KEY) {
		...
		if (!resp.ok) {
			...
			try { lastErr = JSON.parse(errBody).error?.message; } catch { lastErr = `OpenAI ${resp.status}`; }
			resp = null;
		}
	}

	if (!resp && env.OPENROUTER_API_KEY) {
		...
		if (!resp.ok) {
			const errBody = await resp.text();
			try { lastErr = JSON.parse(errBody).error?.message; } catch { lastErr = `OpenRouter ${resp.status}`; }
			resp = null;
		}
	}

	if (!resp) {
		return { result: null, error: lastErr || 'LLM upstream failed', tokenUsage: null };
	}
...
export async function handleAdCommanderDraft(body, env, keyData) {
	...
	const access = await assertAdCommanderAccess(env, ads_account_id, keyData);
	if (!access.ok) {

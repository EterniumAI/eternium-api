...
async function resolveAdsAccount(env, adsAccountId) {
	const qs = new URLSearchParams({
		select: 'id,project_id,projects(id,project_type,tenant_id)',
		id: `eq.${adsAccountId}`,
	});
	...
	return {
		...
		tenant_id: row.projects?.tenant_id,
...
export async function handleCreateCreative(body, env, keyData) {
	...
	const access = await assertAdCommanderAccess(env, body.ads_account_id, keyData.tenant_id);
	if (!access.ok) {
...
export async function handleAutofillCopy(creativeId, body, env, keyData) {
	...
	// Access check
	const access = await assertAdCommanderAccess(env, creative.ads_account_id, keyData.tenant_id);
	if (!access.ok) {
		return { code: access.code, data: { error: access.error, code: access.code === 403 ? 'PROJECT_ACCESS_DENIED' : 'ACCOUNT_NOT_FOUND' } };
	}
...
export async function handleGetActionLog(logId, env, keyData) {
	...
	// Scope by tenant_id directly (per dispatch spec)
	if (log.tenant_id !== keyData.tenant_id) {
...
export async function handleGetMetrics(projectId, adsAccountId, searchParams, env, keyData) {
	// Access check
	const access = await assertAdCommanderAccess(env, adsAccountId, keyData.tenant_id);
	if (!access.ok) {
		return { code: access.code, data: { error: access.error, code: access.code === 403 ? 'PROJECT_ACCESS_DENIED' : 'ACCOUNT_NOT_FOUND' } };
	}
